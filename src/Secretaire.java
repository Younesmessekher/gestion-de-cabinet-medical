...
//planifier rendez-vous pour un patient existant
        ...
        if(index>=0 && index<gestionPt.getPatients().size()) {
            ...
            while (!validInput) {
                ...
                try {
                    dateHeure = LocalDateTime.of(LocalDate.parse(dateStr, getFrDate()), LocalTime.parse(heureStr, getFrHeure()));           
                    if (medecin.disponible(dateHeure)) {
                        ...
                        System.out.println("____Date et Heure non disponible____");
                    }
...
//planifier rendez-vous pour un nouveau patient
    ...
    public void ficheRdV(){
        ...
        if(getListRdV().isEmpty())System.out.println("\n|Pas de rendez vous pour l'instant");
            ...
            for (int index = 0; index < getListRdV().size(); index++) {
                System.out.println("|(" + index + ") " + "Avec : " +getListRdV().get(index).getPatient().getNom()+ " " +getListRdV().get(index).getPatient().getPrenom()+", Le: "+ getListRdV().get(index).getDateHeure().format(getFrDate())+", a: "+getListRdV().get(index).getDateHeure().format(getFrHeure()));
            }
        }   
    }
    ...
    public DateTimeFormatter getFrDate() {
        return frDate;
    }

    public void setFrDate(DateTimeFormatter frDate) {
        this.frDate = frDate;
    }

    public void setFrHeure(DateTimeFormatter frHeure) {
        this.frHeure = frHeure;
    }

    public DateTimeFormatter getFrHeure() {
        return frHeure;
    }
}
